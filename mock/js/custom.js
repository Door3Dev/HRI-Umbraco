!function (a) {
    ...
    e.prototype = {constructor: e, toggle: function (d) {
        var e = a(this), f, g;
...
!function (a) {
    ...
    b.prototype = {constructor: b, validators: {notnull: function (a) {
        return a.length > 0
    }, notblank: function (a) {
        ...
        a.ajax(a.extend({}, {url: c, data: f, type: d.options.remoteMethod || "GET", success: function (a) {
            a = i(a);
    ...
    c.prototype = {constructor: c, init: function (b, c) {
        this.type = c;
        this.valid = true;
        this.element = b;
    ...
    d.prototype = {constructor: d, initMultiple: function (b, c) {
        this.element = b;
        this.$element = a(b);
    ...
    e.prototype = {constructor: e, init: function (b, c, d) {
        this.type = d;
        this.items = [];
        this.$element = a(b);
        this.options = c;
...
var RemoteValidator = function (a) {
    ...
    b.validate = function (c, d, e) {
        ...
        g = c.serialize();
        a.ajax(a.extend({}, {url: h, data: g, type: i, dataType: "json", contentType: "application/json", beforeSend: function (a, c) {
...
(function () {
    $(document).ready(function () {
        ...
        $(".parsley-form").parsley("addListener", {onFieldValidate: function (a) {
            if ($(a).is(":visible")) {
        ...
        var b = {clearRemoteValidations: function (a) {
            ...
            $.ajax($.extend({}, {url: d, data: c, type: "POST", success: function (a) {
                    g(a)
                }, error: function (a) {
                    a = e(a.responseText);
                    f(a)
...
(function () {
    $(document).ready(function () {
        ...
        $(".shop-form").parsley("addListener", {onFieldValidate: function (a) {
            if ($(a).is(":visible")) {
        ...
        $("#shop-form").on("click", "a.shop-next", function (a) {
            a.preventDefault();
            var c = $(this), e = c.closest(".shop-block"), f = e.attr("id"), h = false, j = "";
            if (f === "shop-zipcode") {
                var k = $("#id_zipcode"), h = k.parsley("validate"), l = $("div#shop-form");
                if (h) {
                    RemoteValidator.toggleSpinner("shop-zipcode", {lines: 9, length: 10, width: 5, radius: 10});
                    var m = k.serializeArray();
                    e.data("stepvalid", true);
                    $.ajax($.extend({}, {url: "/shop/ajax/get-counties-by-zip", data: m, dataType: "json", contentType: "application/json", type: "GET", beforeSend: function () {
                            RemoteValidator.clearRemoteValidations(e)
                        }, success: function (a) {
                            var a = RemoteValidator.handleResponse(a);
                            if (a !== undefined) {
                                if (a.status === "success") {
                                    var b = $("#id_shop_county"), f = $("#shop-county"), g = b.find("option"), h = undefined;
                                    g.remove();
                                    h = "<option value>-- Select --</option>";
                                    for (var m = 0; m < a.payloads.length; m++) {
                                        h += '<option value="' + a.payloads[m] + '">' + a.payloads[m] + "</option>"
                                    }
                                    b.append(h);
                                    j = "<p>" + k.val() + "</p>";
                                    i(e, j);
                                    if (a.payloads.length > 1) {
                                        d(c);
                                        f.find(".shop-edit:visible").click()
                                    } else {
                                        e.find(".shop-next").text("Done").hide().end().find(".shop-edit").show();
                                        f.find("#id_shop_county option:eq(1)").prop("selected", true);
                                        if (f.hasClass("shop-block-inactive")) {
                                            f.removeClass("shop-block-inactive")
                                        } else {
                                            f.find(".shop-edit:visible").click()
                                        }
                                        f.find(".shop-next").click().text("Done");
                                        f.find(".shop-next").hide();
                                        f.find(".shop-edit").show()
                                    }
                                } else if (a.status === "failure") {
                                    e.data("stepvalid", false);
                                    RemoteValidator.handleErrors(a, l)
                                } else {
                                    e.data("stepvalid", false)
                                }
                            } else {
                            ...
                            var a = RemoteValidator.handleResponse(a, l)
                        }})).done(function () {
                            RemoteValidator.toggleSpinner("shop-zipcode")
                        })
                } else {
                    "0"
                }
            } else if (f === "shop-county") {
                h = $("#id_shop_county").parsley("validate");
                e.data("stepvalid", h);
                if (h) {
                    j = "<p>" + $("#id_shop_county").val() + "</p>";
                    i(e, j);
                    d(c)
                } else {
                    "0"
                ...
                var n = $("#id_shop_covering_self").prop("checked");
                e.data("stepvalid", true);
                if (n === true) {
                    if (b.tenant === "ny") {
                        h = $("#id_shop_age").parsley("validate")
                    } else {
                        h = $("#id_shop_dob").parsley("validate")
                    }
                    if (h) {
                        var o = 0;
                        if (b.tenant === "ny") {
                            o = parseInt($("#id_shop_age").val(), 10);
                            $("#id_shop_age").val(o);
                            j = "<p>" + o + " years</p>"
                        } else {
                            o = g($("#id_shop_dob").val());
                            j = "<p>" + o + " years (age as of " + hri_planEffectiveDate + " coverage effective date)</p>"
                        }
                        if (o >= 65) {
                            if (hri.getSiteName() === "or") {
                                j = j + "<p><strong>Note: </strong>You have indicated your age as 65 or above. Individuals 65 or older may enroll if they are ineligible for Medicare, they may also enroll in Health Republic Insurance as secondary coverage if they are eligible for Medicare.</p>"
                            } else {
                                j = j + "<p><strong>Note: </strong>You have indicated your age as 65 or above. Individuals 65 or older may enroll only if they are ineligible for Medicare.</p>"
                            }
                        }
                        i(e, j);
                        d(c)
                    ...
                    j = "<p>Not covering self</p>";
                    i(e, j);
                    d(c)
                }
            } else if (f === "shop-smoking") {
                h = $("#shop_smoking").find('input[name="shop_smoking_status"]').parsley("validate");
                if (h) {
                    d(c)
                } else {
                    "0"
                }
            } else if (f === "shop-spouse") {
                var p = $("#id_shop_covering_spouse").prop("checked");
                e.data("stepvalid", true);
                if (p === true) {
                    var q = false;
                    if (b.tenant === "ny") {
                        q = $("#id_shop_spouse_age").parsley("validate")
                    } else {
                        q = $("#id_shop_spouse_dob").parsley("validate")
                    }
                    if (q) {
                        var r = 0;
                        if (b.tenant === "ny") {
                            r = parseInt($("#id_shop_spouse_age").val(), 10);
                            $("#id_shop_spouse_age").val(r);
                            j = "<p>" + r + " years</p>"
                        } else {
                            r = g($("#id_shop_spouse_dob").val());
                            j = "<p>" + r + " years (age as of " + hri_planEffectiveDate + " coverage effective date)</p>"
                        }
                        if (r >= 65) {
                            if (hri.getSiteName() === "or") {
                                j = j + "<p><strong>Note: </strong>You have indicated your spouse/partner's age as 65 or above. Individuals 65 or older may enroll if they are ineligible for Medicare, they may also enroll in Health Republic Insurance as secondary coverage if they are eligible for Medicare.</p>"
                            } else {
                                j = j + "<p><strong>Note: </strong>You have indicated your spouse/partner's age as 65 or above. Individuals 65 or older may enroll only if they are ineligible for Medicare.</p>"
                            }
                        }
                        i(e, j);
                        d(c)
                    ...
                    j = "<p>Not covering spouse</p>";
                    i(e, j);
                    d(c)
                }
            } else if (f === "shop-children") {
                var s = $("#id_shop_covering_children").prop("checked");
                e.data("stepvalid", true);
                if (s === true) {
                    var t = e.find(".children-block"), u = t.length, v = undefined, w = true, j = "<ul>";
                    t.each(function (a) {
                        v = $(this).find(".dependent-age").parsley("validate");
                        w = w && v;
                        if (v) {
                            if (b.tenant === "ny") {
                                var c = parseInt($(this).find(".dependent-age").val());
                                $(this).find(".dependent-age").val(c);
                                j += "<li><p>" + c + " years</p></li>"
                            } else {
                                var c = g($(this).find(".dependent-age").val());
                                j += "<li><p>" + c + " years (age as of " + hri_planEffectiveDate + " coverage effective date)</p></li>"
                            }
                        }
                    });
                    if (w) {
                    ...
                    j = "<p>Not covering children</p>";
                    i(e, j);
                    d(c)
                }
            } else if (f === "shop-salary") {
                if ($("#id_shop_salary").val()) {
                    h = $("#id_shop_salary").parsley("validate");
                    e.data("stepvalid", h);
                    if (h) {
                        j = "<p>$" + $("#id_shop_salary").val() + "</p>";
                        i(e, j);
                        d(c)
                    } else {
                        "0"
                    ...
                    e.data("stepvalid", "true");
                    j = "<p>Not Applicable</p>";
                    i(e, j);
                    d(c)
                }
            } else if (f === "shop-household") {
                if ($("#id_shop_household").val()) {
                    h = $("#id_shop_household").parsley("validate");
                    e.data("stepvalid", h);
                    if (h) {
                        j = "<p>" + $("#id_shop_household").val() + "</p>";
                        i(e, j);
                        d(c);
                        e.data("stepvalid", h)
                    } else {
            ...
            } else if (f === "shop-subsidy") {
                if ($("#id_shop_salary").val() && $("#id_shop_household").val()) {
                    var x = $("#id_shop_salary").parsley("validate");
                    isHouseholdValid = $("#id_shop_household").parsley("validate");
                    h = x && isHouseholdValid;
                    e.data("stepvalid", h);
                    if (h) {
                        var y = parseInt($("#id_shop_salary").val());
                        subsidy_household = parseInt($("#id_shop_household").val());
                        $("#id_shop_salary").val(y);
                        $("#id_shop_household").val(subsidy_household);
                        j = "<p>$" + y + "</p><p>" + subsidy_household + " person(s)</p>";
                        i(e, j);
                        d(c)
                    } else {
                        "0"
        ...
        var d = function (a) {
            a.hide().text("Done").next(".shop-edit").show();
            e(a)
        };
        var e = function (a) {
            var b = a.closest(".shop-block").next(".shop-block");
            if (b.hasClass("shop-block-inactive") && b.attr("id") != "shop-submit") {
                b.css({opacity: "0"}).removeClass("shop-block-inactive").animate({opacity: 1}, 600, function () {
                })
            } else {
            }
        };
        ...
        var j = function () {
            var a = $("#shop-systemmessaging"), b = a.find(".systemmessage"), c = $("#id_shop_covering_self").prop("checked"), d = $("#id_shop_covering_spouse").prop("checked"), e = $("#id_shop_covering_children").prop("checked");
            if (d) {
                var f = k();
                if (f.status) {
                    if (f.message === null) {
                    } else {
                        a.removeClass("shop-block-inactive system-invalid");
                        b.html(g.message || "");
                        $("a.shop-submit").hide();
                        return false
                    }
                } else {
                    a.removeClass("shop-block-inactive").addClass("system-invalid");
                    b.html(f.message || "");
                    $("a.shop-submit").hide();
                    return false
                }
            ...
            if (!c && !d && !e) {
                a.removeClass("shop-block-inactive").addClass("system-invalid");
                b.html("The information you have entered is incomplete. Please enter details of the person you want to cover and submit the form again.");
                $("a.shop-submit").hide();
                return false
            }
            var h = $("form.shop-form");
            RemoteValidator.validate(h, "shop-form", true)
        };
        var k = function () {
            var a = $("#id_shop_covering_self").prop("checked"), b = $("#id_shop_covering_spouse").prop("checked"), c = {};
            if (!a && b) {
                c.message = "You've indicated that you want to cover your spouse/partner on this policy. If you are purchasing coverage on behalf of your spouse/partner, their information should be entered in the 'Self' section (please contact Member Services at 1-888-990-6635 with any questions).";
                c.status = false;
                return c
            } else {
                c.message = null;
        ...
        var l = function () {
            var a = $("#id_shop_covering_children").prop("checked"), c = $("#id_shop_covering_self").prop("checked"), d = $("#shop-children"), e = d.find(".children-block"), f = [], h = true, i = false, j = false, k = false, l = {};
            e.each(function (a) {
                if (b.tenant === "ny") {
                    f.push(parseInt($(this).find(".dependent-age").val(), 10))
                } else {
                    f.push(g($(this).find(".dependent-age").val()))
                }
                if (f[a] >= 21) {
                    h = false
                }
            ...
            if (a && !c) {
                if (h) {
                    l.message = "Since all children added above are below 21 years of age, they will be enrolled on a Child-Only policy";
                    l.status = true;
                    return l
                } else {
                    l.message = "Since one or more children added are 21 years of age or older, you must enroll them as 'Subscriber' on their own policy. Please enter their information in the 'Self' section. Only children under 21 years of age can be included in a Child-Only policy.";
                    l.status = false;
                    return l
                }
            } else if (a && c) {
                if (b.tenant === "nj") {
                    if (i) {
                        l.message = "Dependents age 26 years of age and older must be enrolled on their own policy (unless they qualify as legally disabled).";
                        l.status = true;
                        return l
                    }
                }
                ...
                if (b.tenant === "ny") {
                    if (k) {
                        l.message = "In NY, children between the ages of 26 and 29 may be added as dependents only if they meet certain requirements: 1) they are not eligible for employer-sponsored coverage, 2) they live or work in NY state, and 3) they are unmarried.";
                        l.status = true;
                        return l
                    } else if (j) {
                        l.message = "In NY, dependents age 30 or older must be enrolled on their own policy (unless they qualify as legally disabled).";
                        l.status = true;
                        return l
                    }
                }
        ...
        $("#shop-form").on("click", "a.shop-continue", function (a) {
            a.preventDefault();
            var b = $(this);
            var c = $(this).closest("form.shop-form");
            RemoteValidator.validate(c, "shop-form", true)
        });
        $("#shop-form").on("click", "a.shop-submit", function (a) {
            a.preventDefault();
            var b = $(this);
            if (b.hasClass("disabled"))return;
            j()
        });
        $("#shop-form").on("click", "a.shop-edit", function (a) {
            a.preventDefault();
            var b = $(this), c = b.closest(".shop-block");
            c.data("stepvalid", false);
            h();
            b.hide().prev(".shop-next").show();
            i(c);
            $("#shop-systemmessaging").addClass("shop-block-inactive");
            $("a.shop-submit").show()
        });
        $("#shop-form").on("click", "a.shop-next", function (a) {
            a.preventDefault();
        ...
        $("#shop-form").on("keypress", "div.shop-block", function (a) {
            if (a.which === 13 || a.keyCode === 13) {
                $(this).find("a.shop-next:visible").click()
            }
        });
        $("#shop-form").on("click", "input#id_shop_covering_self", function (a) {
            if ($(this).prop("checked")) {
                $("#covering-self").removeClass("hri-hide")
            } else {
                if (b.tenant === "ny") {
                    $("#covering-self").addClass("hri-hide").find("input#id_shop_age").val("")
                } else {
                    $("#covering-self").addClass("hri-hide").find("input#id_shop_dob").val("")
                }
            }
        });
        $("#shop-form").on("click", "input#id_shop_covering_spouse", function (a) {
            if ($(this).prop("checked")) {
                $("#covering-spouse").removeClass("hri-hide")
            } else {
                if (b.tenant === "ny") {
                    $("#covering-spouse").addClass("hri-hide").find("input#id_shop_spouse_age").val("")
                } else {
                    $("#covering-spouse").addClass("hri-hide").find("input#id_shop_spouse_dob").val("")
                }
            }
        });
        $("#shop-form").on("click", "input#id_shop_covering_children", function (a) {
            if ($(this).prop("checked")) {
                $("#covering-children").removeClass("hri-hide")
            } else {
                var b = $("#covering-children").closest(".shop-block");
                $("#covering-children").addClass("hri-hide").find(".children-block").remove();
                $("#id_dependent-TOTAL_FORMS").val(0);
                b.data("dependents-counter", 0);
                b.find("a.add-child").click()
            }
        });
        $("#shop-form").on("click", "a.add-child", function (a) {
            a.preventDefault();
            var c = $(this);
            var d = c.closest(".shop-block"), e = d.find("#children"), f = e.find(".children-block"), g = f.length, h = parseInt(d.data("dependents-counter"));
            $("#id_dependent-TOTAL_FORMS").val(g + 1);
            d.data("dependents-counter", parseInt(d.data("dependents-counter")) + 1);
            if (b.tenant === "ny") {
                e.append('<div id="id_dependent-' + h + '-row" class="children-block">' + '<div class="row-fluid">' + '<div class="span8"><label>How old is your child?' + '<input id="id_dependent-' + h + '-shop_dependent_age" class="span10 dependent-age" ' + 'name="dependent-' + h + '-shop_dependent_age" type="text" maxlength="3" data-required="true" data-regexp="^\\d{1,3}$" ' + 'data-range="[0,110]" data-regexp-message="This is not a valid age">' + "</label></div>" + '<div class="span3">' + '<a href="#" class="shop-link remove-child"><i class="icon-minus-sign"></i>Remove child</a>' + "</div>" + "</div></div>")
            } else {
                e.append('<div id="id_dependent-' + h + '-row" class="children-block">' + '<div class="row-fluid">' + '<div class="span8"><label>Date of Birth <small class="dark-purple"><em>(YYYY-MM-DD)</em></small>' + '<input id="id_dependent-' + h + '-shop_dependent_dob" class="span10 dependent-age" ' + 'name="dependent-' + h + '-shop_dependent_dob" type="text" data-required="true" data-regexp="^\\d{4}-\\d{2}-\\d{2}$" ' + 'data-regexp-message="This is not a valid date of birth">' + "</label></div>" + '<div class="span3">' + '<a href="#" class="shop-link remove-child"><i class="icon-minus-sign"></i>Remove child</a>' + "</div>" + "</div></div>")
            }
        });
        $("#shop-form").on("click", "a.remove-child", function (a) {
            var b = $(this).closest(".shop-block");
            $childBlock = $(this).closest("div.children-block");
            a.preventDefault();
            var c = $childBlock.find("input").attr("id");
            $("#shop-form>form").parsley("removeItem", "#" + c);
            $childBlock.remove();
            var d = b.find(".children-block"), e = d.length;
            if (!e) {
                $("#id_shop_covering_children").trigger("click")
            }
            $("#id_dependent-TOTAL_FORMS").val(e)
        })
    })
})();
(function (a) {
...
(function () {
    $(document).ready(function () {
        var a = {$flowbar: $(".flowbar"), flowmessage: false, flowseconds: 0, flowtimer: "", init: function () {
...
(function () {
  $(document).ready(function () {
    ...
    $('#shop-zipcode').autocomplete({
      //serviceUrl: 'some http request for zip codes list'
      lookup: ['10001', '10002', '10003', '10004'], //TODO: change it to work with service url
      onSelect: function (suggestion) {
        $('#shop-county').text('Chemung').show(); //TODO: add server side logic to find county by zip code
      }
    });
